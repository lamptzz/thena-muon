...
const PERP_MANAGER_ABI = [
    {
        ...
        name: 'idToTradingCompetitionAddress',
...
module.exports = {
    ...
    _getTradingCompetitionAddress: async function (tcId) {
        const tradingCompetition = await ethCall(
            perpManagerAddress,
            'idToTradingCompetitionAddress',
            [tcId],
            PERP_MANAGER_ABI,
            defaultChainId
        )
    ...
    getWeightsLen: async function (tcId) {
        ...
        const weightsLen = await ethCall(
            tc_address,
            'getWeightsLength',
            [],
            ACCOUNT_MANAGER_ABI,
    ...
    getPositionAndTieCounter: async function (participants, owner, tcId) {
        ...
        const validParticipants = participants.filter(
            (participants) => participants.isValid && participants.isValidDeallocate
        )

        // Find if owner is valid
        const isOwnerValid = validParticipants.some(
            (participant) => participant.owner.toLowerCase() === owner.toLowerCase()
        )
        if (!isOwnerValid) throw { message: 'OWNER_NOT_VALID' }

        // Sort all participants
        const sortedParticipants = validParticipants.sort(
            (a, b) => parseFloat(b.percentagePnl) - parseFloat(a.percentagePnl)
        )

        // Find the index of the given owner
        const ownerIndex = sortedParticipants.findIndex(
            (participants) => participants.owner.toLowerCase() === owner.toLowerCase()
        )
        ...
        const firstIndexSamePnl = sortedParticipants.findIndex(
            (participant) => participant.percentagePnl === ownerPnl
        )

        // Find all participants with the same percentagePnl as owner
        const samePnlParticipants = sortedParticipants.filter(
            (participant) => participant.percentagePnl === ownerPnl
        )
        ...
        const higherParticipants = sortedParticipants.filter(
            (participant) => parseFloat(participant.percentagePnl) > parseFloat(ownerPnl)
        )
        ...
        if (firstIndexSamePnl > weightslen || higherParticipants.length >= weightslen)
            throw { message: 'OWNER_NOT_WIN' }
    ...
    getInfo: async function (tcId) {
        const subgraphEndpoint =
            'https://api.studio.thegraph.com/query/70764/tc-perp-subgraph/version/latest'
        const query = `{
    ...
    _info: async function (owner, tcId) {
        ...
        // find position
        const { position, tiecounter } = await this.getPositionAndTieCounter(
            participants,
            owner,
            tcId
        )
    ...
    onRequest: async function (request) {
        ...
        switch (method) {
            case 'position': {
                // gets required info
                const { position, tiecounter } = await this._info(owner, tcId)
