const MuonAppUtils = require('../muonapp-utils')
...
const DIBS_REPO_ABI = [
    {
        ...
        outputs: [
            ...
            { internalType: 'address', name: 'dibs', type: 'address' },
            { internalType: 'string', name: 'subgraphEndpoint', type: 'string' },
            { internalType: 'uint32', name: 'firstRoundStartTime', type: 'uint32' },
            { internalType: 'uint32', name: 'roundDuration', type: 'uint32' },
...
module.exports = {
    ...
    fetchProject: async function (projectId) {
        const { dibs, chainId, subgraphEndpoint } = await ethCall(
            DibsRepository,
            'projects',
            [projectId],
            DIBS_REPO_ABI,
    ...
    signParams: function (request, result) {
        ...
        switch (method) {
            case 'userVolume': {
                let { projectId, user, pair, day, userVolume, totalVolume } = result

                if (
                    !this.isToleranceOk(userVolume, request.data.result.userVolume, VALID_TOLERANCE)
                        .isOk
                )
                ...
                if (
                    !this.isToleranceOk(
                        totalVolume,
                        request.data.result.totalVolume,
                        VALID_TOLERANCE
                    ).isOk
                )
